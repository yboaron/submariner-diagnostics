...
|------------------|----------------|-----------|--------|
| 0 | 0 | LoadBalancer/firewall blocking | Check LB backend pool, security groups, firewall rules |
| > 0 | 0 | **OVN forwarding failure** | Restart ovnkube-node, check OVN load balancer, clear conntrack |
| > 0 | > 0 | Normal operation | Traffic flowing correctly |
| 0 | > 0 | Unexpected | Check if direct pod access bypassing NodePort |

**OVN forwarding failure remediation:**

If NodePort traffic arrives but tunnel traffic is 0:

```bash
# 1. Restart ovnkube-node on gateway node
oc delete pod -n openshift-ovn-kubernetes -l app=ovnkube-node --field-selector spec.nodeName=<gateway-node>

# 2. Check OVN load balancer configuration
oc debug node/<gateway-node> -- chroot /host ovn-nbctl list load_balancer | grep submariner

# 3. Check OVS flows
oc debug node/<gateway-node> -- chroot /host ovs-ofctl dump-flows br-int | grep -E '4500|4490'

# 4. Clear conntrack entries
oc debug node/<gateway-node> -- chroot /host conntrack -F

# 5. Last resort: restart node/VM
```

**LoadBalancer/firewall issue remediation:**

If no NodePort traffic arrives:

```bash
# 1. Verify LoadBalancer backend pool
# (Cloud provider specific - check LB configuration includes cluster nodes)

# 2. Check security groups allow UDP to NodePorts
# (Cloud provider specific - ensure UDP allowed on NodePort range)

# 3. Verify firewall rules between LoadBalancers
# Run firewall inter-cluster test if available

# 4. Check LoadBalancer health checks
# Ensure health checks don't prevent traffic forwarding
```
