...
EOF
    ...
    kubectl exec -n submariner-operator "${TCPDUMP_POD}" --kubeconfig="${kubeconfig}" --context="${context}" -- cat /tmp/gateway-traffic.pcap > "${tcpdump_dir}/${cluster_name}-gateway-${GATEWAY_NODE}.pcap" 2>&1

    # Extract analysis file
    kubectl exec -n submariner-operator "${TCPDUMP_POD}" --kubeconfig="${kubeconfig}" --context="${context}" -- cat /tmp/gateway-analysis.txt > "${tcpdump_dir}/${cluster_name}-gateway-${GATEWAY_NODE}-analysis.txt" 2>&1
