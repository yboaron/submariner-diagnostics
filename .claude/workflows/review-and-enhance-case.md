...
This workflow is **automatically available** when you're in the submariner-diagnostics directory with Claude Code. See [learn-from-investigation.md](learn-from-investigation.md) for setup details.

**TL;DR:** Just clone the repo, `cd` into it, and type `/workflow review-and-enhance-case cases/review/case-XXX.yaml`
...
if (!caseFilePath) {
  throw new Error('Usage: /workflow review-and-enhance-case cases/review/case-XXX.yaml')
}

phase('Validate')

log(`Reading case file: ${caseFilePath}`)

const caseFile = await agent(
  `Read and parse case file: ${caseFilePath}

  Validate:
  - All required fields present
  - At least 3 key indicators
  - Detection logic is clear
  ...
  RETURN:
  {
    valid: true/false,
    issues: [...],
    case_data: { ... parsed YAML ... }
  }`,
  { phase: 'Validate' }
)

if (!caseFile.valid) {
  log('❌ Case file validation failed:')
  caseFile.issues.forEach(issue => log(`  - ${issue}`))
  throw new Error('Fix validation issues before proceeding')
}

const caseData = caseFile.case_data
const caseId = caseData.metadata.case_id

log(`✓ Case validated: ${caseId}`)
log(`  Root cause: ${caseData.root_cause.summary}`)
log(`  Indicators: ${caseData.indicators.required.length} required, ${caseData.indicators.optional.length} optional`)
...
const testDiagnostic = await agent(
  ...
  Create directory structure: tests/diagnostics/case-${caseId}/
  ...
  GENERATE:
  {
    base_dir: "tests/diagnostics/case-${caseId}",
    files: [
      { path: "...", content: "..." },
      ...
    ],
...
log(`✓ Created test diagnostic: ${testDiagnostic.base_dir}`)
...
const regressionTest = await agent(
  ...
  Find: tests/diagnostics/*/
  Run on each (exclude new case-${caseId})
...
// Apply code changes (this would be done via file writes in real implementation)
// For now, document what needs to be written

log('Applying code changes...')
log(`  - analyze-basic.py (add detection function)`)
log(`  - ${docsUpdate.file_path}`)
log(`  - tests/${pythonTests.test_file_name}`)
log(`  - ${testDiagnostic.base_dir}/* (test diagnostic files)`)
