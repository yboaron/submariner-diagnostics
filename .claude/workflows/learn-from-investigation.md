...
if (validation.validated_indicators.filter(i => !i.found).length > 0) {
  ...
  validation.validated_indicators
    .filter(i => !i.found)
    .forEach(i => log(`  - ${i.name}: Not found at ${i.file_location}`))
...
// Assign case ID
const caseId = `${String(Date.now()).slice(-3)}-${caseInfo.root_cause.summary
  .toLowerCase()
  .replace(/[^a-z0-9]+/g, '-')
  .replace(/^-|-$/g, '')
  .slice(0, 40)}`
...
root_cause:
  ...
  explanation: |
    ${caseInfo.root_cause.detailed_explanation}
...
indicators:
  required:
${validation.validated_indicators.filter(i => i.required).map(ind => `    - name: "${ind.name}"
      file_location: "${caseInfo.key_indicators.find(k => k.name === ind.name)?.file_location}"
      actual_path: "${ind.actual_file_path}"
      pattern: "${caseInfo.key_indicators.find(k => k.name === ind.name)?.pattern}"
      ${caseInfo.key_indicators.find(k => k.name === ind.name)?.regex_pattern ? `regex: "${caseInfo.key_indicators.find(k => k.name === ind.name)?.regex_pattern}"` : ''}
      significance: "${caseInfo.key_indicators.find(k => k.name === ind.name)?.significance}"
      excerpt: |
...
${validation.validated_indicators.filter(i => !i.required).map(ind => `    - name: "${ind.name}"
      ...
      actual_path: "${ind.actual_file_path}"
      pattern: "${caseInfo.key_indicators.find(k => k.name === ind.name)?.pattern}"
      significance: "${caseInfo.key_indicators.find(k => k.name === ind.name)?.significance}"`).join('\n\n')}
...
// Write case file
const caseFilePath = `cases/review/case-${caseId}.yaml`
